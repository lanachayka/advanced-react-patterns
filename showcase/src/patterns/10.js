import React, { useState, useLayoutEffect, useCallback, useRef, useEffect, useReducer } from 'react'
...
const useClapAnimation = ({ clapEl, clapCountEl, clapCountTotalEl }) => {
    ...
    useLayoutEffect(() => {
        if (!clapEl || !clapCountEl || !clapCountTotalEl) {
            return
        }
        ...
        const traingleBurst = new mojs.Burst({
            parent: clapEl,
            radius: { 50: 95 },
            count: 5,
            angle: 30,
...
// custom hook to get PrevProps or State
...
const useClapState = (initialState = INITIAL_STATE, reducer = internalReducer) => {
    ...
    return { clapState, getTogglerProps, getCounterProps, reset, resetDep: resetRef.current }
}
...
const Usage = () => {
    ...
    const isClappedTooMatch = timesClapped >= 7
    const userReducer = (state, action) => {
        if (action.type === useClapState.types.clap && isClappedTooMatch) {
            return state
        }
        return useClapState.reducer(state, action)
    }

    const { clapState, getTogglerProps, getCounterProps, reset, resetDep } = useClapState(userInitialState, userReducer)
    ...
    return (
        <div>
            <ClapContainer
                setRef={setRef}
                {...getTogglerProps({ handleClick: userHandleClick })}
                data-refkey='clapRef'
            >
                <ClapIcon isClicked={isClicked} />
                <ClapCount {...getCounterProps()} setRef={setRef} data-refkey='clapCountRef' />
                <ClapTotal countTotal={countTotal} setRef={setRef} data-refkey='clapCountTotalRef' />
            </ClapContainer>
            <section>
                ...
                <pre className={userStyles.resetBtn}>
                    {uploadingReset ? `uploadingReset ${resetDep} ...` : `...`}
                </pre>
                <pre style={{color: 'red'}}>
                    {isClappedTooMatch ? `You have clapped too match. Don't be so generous!` : ''}
                </pre>
            </section>
        </div>
    )
